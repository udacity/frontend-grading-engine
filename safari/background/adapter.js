...
/**
 * @fileOverview This file contains the adaptee (Adapter inner
 ...
 * @namespace
 * @property {error} wrapper.runtime.lastError - Set for the lifetime
 * of a callback if an ansychronous extension api has resulted in an
 * error. If no error has occured lastError will be undefined.
 */
var wrapper = {
  storage: {
    sync: {
...
/**
 * Emulates the chrome storage behavior (getter) by using the
 * {@link safari.extension.settings} mechanism.
 * @param {string|string[]|object} keys - A single key to get, list of
 * keys to get, or a dictionary specifying default values (see
 ...
 * @throws {error} Error in the {@link keys} argument and sets
 * {@link wrapper.runtime.lastError}.
 */
wrapper.storage.sync.get = function(keys) {
  var i, len, key, items = {};
  try {
    if(!keys) {
      if(keys === null) {
        ...
        // Only `null` can return values, otherwise it’s an empty
        // Object.
        items = {};
      }
      ...
      for(i=0, len=keys.length; i<len; i++) {
        ...
        if(!(key instanceof String || typeof key === 'string')) {
          extensionLog(new Error('An item of the `keys` array' +
                                 'wasn’t a String'));
        }
      ...
      if(keysArray.length === 0) {
        extensionLog(new Error('The `keys` object does not contain ' +
                               'any property on its own'));
      }

      for(i=0, len=keysArray.length; i<len; i++) {
        key = keysArray[i];
        value = safari.extension.settings[key];
        // Return the default value if the key isn’t present in
        // settings.
...
/**
 * Emulates the chrome storage behavior (setter) by using the
 * {@link safari.extension.settings} mechanism.
 * @param {*} keys - An object which gives each key/value pair to
 ...
 * with a typeof {@link Object} and {@link Function} will typically
 * serialize to `{}`, with the exception of {@link Array} (serializes
 * as expected), {@link Date}, and {@link Regex} (serialize using
 * their {@link String} representation).
 * @returns {int} 0 on success and -1 on error.
 * @throws {error} Error in the {@link keys} argument and sets
 * {@link wrapper.runtime.lastError}.
 */
wrapper.storage.sync.set = function(keys) {
  try {
    if(!keys || keys instanceof String ||
       typeof keys === 'string' ||
       keys instanceof Array) {
      extensionLog(new Error('The `keys` argument is not a valid ' +
                             'Object with keys/properties'));
    }
...
/**
 * Sends a single message to the content script(s) in the specified
 * tab, with an optional callback to run when a response is sent
 * back. The {@link injected.runtime.onMessage} event is fired in each
 * content script running in the specified tab for the current
 * extension.
 * @param {int} tabId - The tab to send the message to.
 * @param {*} message - Any object that can be serialized.
 * @returns {Promise} A promise to be fulfilled when it has been
 * received.
 */
wrapper.tabs.sendMessage = function(tabId, message, options, channel) {
  try {
    var tab = registry.getTabById(tabId);

    channel = channel || Math.floor(Math.random() * 100000000);
    tab.page.dispatchMessage('injected.runtime.onMessage',
                             JSON.stringify({
                               message: message,
                               channel: channel
                             }));
 ...
 * @param {bool} [queryInfo.active] - TODO Whether the tabs are active
 * in their windows. (Does not necessarily mean the window is
 * focused.)
 * @param {bool} [queryInfo.currentWindow] - TODO Whether the tabs are
 * in the /current window/. Note: the current window doesn’t mean it’s
 * the active one. It means that the window is currently executing.
 * @returns {int|Object[]} Result of the query of -1 on error.
 */
wrapper.tabs.query = function(queryInfo) {
  /**
   * Gets all active {@link SafariBrowserTab} from given an array
   * {@link SafariBrowserWindow}.
   * @param {SafariBrowserWindow[]} windows - Windows to get all
   * active tabs.
  ...
  function activeTabs(windows) {
    ...
    for(len=windows.length; i<len; i++) {
  ...
  function getTabs(windows) {
    ...
    for(len = windows.length; i < len; i++) {
      ...
      for(u = 0, uLen = windowTabs.length; u < uLen; u++) {
  ...
  try {
    ...
    if(Object.prototype.toString.call(queryInfo) ===
       '[object Object]') {
      // queryInfo.currentWindow
      if(queryInfo.currentWindow) {
        /** Because there’s no way I know to select the window
         * currently running in Safari, the active window (or
         * {@link lastFocusedWindow} one if null) will be used
         * instead. If someone successfully thriggers an action page
         * that isn’t focused, it’s an undefined behavior.
         */
