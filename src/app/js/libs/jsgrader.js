/**
 * @fileOverview This file contains the JSGrader library to test the JavaScript
 * context.
...
var Grader = (function() {
  ...
  function deepCompare () {
    ...
    function compare2Objects (x, y) {
      ...
      // remember that NaN === NaN returns false and isNaN(undefined) returns
      // true
      if (isNaN(x) && isNaN(y) && typeof x === 'number' && typeof y === 'number') {
        return true;
      }

      // Compare primitives and functions.  Check if both arguments link to the
      // same object.  Especially useful on step when comparing prototypes
      if (x === y) {
        return true;
      }

      // Works in case when functions are created in constructor.  Comparing
      // dates is a common scenario. Another built-ins?  We can even handle
      // functions passed across iframes
    ...
    if (arguments.length < 1) {
      // Die silently? Don’t know how to handle such case, please
      // help...
      return true;
